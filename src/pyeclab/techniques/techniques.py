...
def make_CA_ecc_params(api, parameters):
    ...
    p_voltage_steps.append(make_ecc_parm(api, ca_param_names["voltage_step"], parameters.voltage, idx))
    p_voltage_steps.append(make_ecc_parm(api, ca_param_names["step_duration"], parameters.duration, idx))
    p_voltage_steps.append(make_ecc_parm(api, ca_param_names["vs_init"], parameters.vs_init, idx))
    p_nb_steps = make_ecc_parm(api, ca_param_names["nb_steps"], parameters.nb_steps)
    p_record_dt = make_ecc_parm(api, ca_param_names["record_dt"], parameters.record_dt)
    p_record_dI = make_ecc_parm(api, ca_param_names["record_dI"], parameters.record_dI)
    p_xctr = make_ecc_parm(api, ca_param_names["xctr"], parameters.xctr)
    p_repeat = make_ecc_parm(api, ca_param_names["repeat"], parameters.repeat)
    p_IRange = make_ecc_parm(api, ca_param_names["i_range"], parameters.i_range)
    p_ERange = make_ecc_parm(api, ca_param_names["e_range"], parameters.e_range)
    p_band = make_ecc_parm(api, ca_param_names["bandwidth"], parameters.bandwidth)
    # make the technique parameter array
