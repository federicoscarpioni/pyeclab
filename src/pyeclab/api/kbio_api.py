...
class KBIO_api:
    ...
    def GetData(self, id_, ch):
        pb = KBIO.DataBuffer()  # DataBuffer = c_uint32 * 1000 form kbio_types.py module
        di = KBIO.DataInfo()
        cv = KBIO.CurrentValues()
        ...
        logger.debug(f"Getting DataBuffer in kbio:\n {db=}")
        logger.debug(f"Getting DataInfo in kbio:\n {di=}")
        logger.debug(f"Getting CurrentValues in kbio:\n {cv=}")
    ...
    class ChannelInfo(KBIO.ChannelInfo):
        ...
        def __str__(self):
            ...
            if self.has_no_firmware:
                ...
                if memsize:
                    fragments.append(
                        f"Memory: {self.MemSize/1024:.1f}KB" f" ({(self.MemFilled/self.MemSize*100.):.2f}% filled)"
                    )
                else:
